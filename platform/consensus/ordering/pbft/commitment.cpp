 ...
 */
 ...
 int Commitment::ProcessNewRequest(std::unique_ptr<Context> context,
   ...
   if (need_signature &&
       (context == nullptr || context->signature.signature().empty())) {
 ...
 int Commitment::ProcessProposeMsg(std::unique_ptr<Context> context,
   ...
   if (request->sender_id() != config_.GetSelfInfo().id()) {
    ...
    // check signatures only when signature verification is enabled.
    if (need_signature) {
      bool valid =
   ...
   if (ret == CollectorResultCode::STATE_CHANGED) {
    // Add the local PREPARE vote directly to local state so consensus does not
    // depend on network loopback-to-self delivery.
    SignatureInfo local_sig;
    message_manager_->AddConsensusMsg(local_sig,
                                      std::make_unique<Request>(*prepare_request));
